...
class ARPOD_GYM(gym.Env):
    ...
    def __init__(self):
        ...
        os.mkdir(f'Benchmark/velocities/{self.vels_dir}')

        model_idu = len(os.listdir('Benchmark/actuations/'))
        self.u_dir = f'vbar{model_idu}'
        print(f'creating actuation logs in directory {self.u_dir}')
        os.mkdir(f'Benchmark/actuations/{self.u_dir}')

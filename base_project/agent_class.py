...
class agent_base():
    ...
    def evaluate_stopping_criterion(self,list_of_returns):
        ...
        #print("EVALUATE!!!!!!!!!!")
        #print(self.n_solving_episodes)
        #print(len(list_of_returns))
        ...
        # self.n_solving_episodes episodes 
    ...
    def train(self,environment,
        ...
        for n_episode in range(self.n_episodes_max):
            #print("EPISODE")
            #print(n_episode,self.n_episodes_max)
            #
            ...
            for i in itertools.count(): # timesteps of environment
                ...
                if step_counter % self.training_stride == 0:
                    ...
                    #print(step_counter,epoch_counter)
                #
                if done: # if current episode ended
                    ...
                    #print(min_ret)
                    #print(mean_ret)
                    # min_ret = 0, mean_ret = 0, training_complete = False: ALWAYS!!! WHY??????
                    if verbose:
                            #print("VERBOOOOOSE")
            ...
            #print("NEEEEEEEEEEWWWWWWWW")
            #print(n_episode,self.saving_stride,n_episode%self.saving_stride)
             ...
             #       or training_complete \
              #      or n_episode == self.n_episodes_max-1:
                ...
                if training_filename != None:
                    #Problem in the save_dictionary function.
                    #print("NOOOOOOONREEEEEEEEEEEEEE")
    ...
    def save_dictionary_recursively(self, h5file, path, dictionary):
        for key, value in dictionary.items():
            ...
            # Debugging statement
            print(f"Processing key: {full_path}, Value: {value}, Type: {type(value)}")

            if isinstance(value, dict):
                # Recursively save sub-dictionaries
                self.save_dictionary_recursively(h5file, full_path + '/', value)
            else:
                try:
                    # Handle lists
                    if isinstance(value, list):
                        if len(value) == 1:
                            # Single-element list, store as scalar
                            value = value[0]
                        else:
                    ...
                    if isinstance(value, (int, np.integer)):
                        ...
                        # Ensure array does not have object dtype
                        if value.dtype == np.dtype('O'):
                            value = value.astype(str)
                            value = np.array(value, dtype=h5py.string_dtype(encoding='utf-8'))
                    elif value is None:
                        value = np.array("None", dtype=h5py.string_dtype(encoding='utf-8'))
                    else:
                        # Convert other types to string
                        value = np.array(str(value), dtype=h5py.string_dtype(encoding='utf-8'))

                    # Debugging statement
                    print(f"Saving key: {full_path}, Data: {value}, Dtype: {getattr(value, 'dtype', type(value))}, Shape: {getattr(value, 'shape', 'scalar')}")

                    # Save the data
                    h5file.create_dataset(full_path, data=value)
                except Exception as e:
                    print(f"Error saving key: {full_path}")
                    print(f"Exception: {e}")
                    raise
    ...
    #def save_dictionary_recursively(self, h5file, path, dictionary):
     ...
     #   for key, value in dictionary.items():
      ...
      #      key = str(key)
       ...
       #     if isinstance(value, dict):
                # Recursively save sub-dictionaries
        #        self.save_dictionary_recursively(
         #           h5file,
          #          path + key + '/',
           #         value
            #    )
            #else:
                # Handle empty arrays or lists
             #   if isinstance(value, (list, np.ndarray)) and len(value) == 0:
                    # Store an empty dataset with appropriate dtype
              #      h5file.create_dataset(path + key, data=np.array([]))
               #     continue

                # Handle scalar numeric and boolean values
                #elif isinstance(value, (int, float, complex, bool, np.integer, np.floating, np.bool_)):
                 #   data = np.array(value)
                # Handle lists
                ...
                # Handle strings
                 ...
                 #   string_dtype = h5py.string_dtype(encoding='utf-8')
                  #  data = np.array([value], dtype=string_dtype)
                # Handle bytes
                ...
                #elif value is None:
                 #   string_dtype = h5py.string_dtype(encoding='utf-8')
                  #  data = np.array(["None"], dtype=string_dtype)
                #else:
                    # Convert other types to their string representation
                 #   string_dtype = h5py.string_dtype(encoding='utf-8')
                  #  data = np.array([str(value)], dtype=string_dtype)

                # Save the data
                #h5file[path + key] = data
              ...
              #  if isinstance(value, (list, np.ndarray)) and len(value) == 0:
               ...
               #     h5file.create_dataset(path + key, data=np.array([]))
                ...
                # Handle scalar values
                 ...
                 #   data = np.array(value)
                #elif isinstance(value, str):
                    # Use variable-length Unicode strings
                 #   string_dtype = h5py.string_dtype(encoding='utf-8')
                  #  data = np.array(value, dtype=string_dtype)
                #elif isinstance(value, bytes):
                    # Use variable-length byte strings
                 #   string_dtype = h5py.special_dtype(vlen=bytes)
                  #  data = np.array(value, dtype=string_dtype)
               # elif value is None:
                    # Represent None as a string
                #    string_dtype = h5py.string_dtype(encoding='utf-8')
                 #   data = np.array("None", dtype=string_dtype)
                #elif isinstance(value, np.ndarray):
                    # Ensure arrays have supported dtypes
                 #   data = value
                #else:
                    # Convert other types to their string representation
                 #   string_dtype = h5py.string_dtype(encoding='utf-8')
                ...
                #if isinstance(data, np.ndarray) and data.dtype.kind == 'S' and data.dtype.itemsize == 0:
                    # Adjust dtype to variable-length string
                 #   string_dtype = h5py.string_dtype(encoding='utf-8')
                  #  data = np.array(data, dtype=string_dtype)
    ...
    def save_dictionary_recursively(self, h5file, path, dictionary):
        ...
        for key, value in dictionary.items():
            ...
            if isinstance(value, dict):
                # Recursively save sub-dictionaries
                self.save_dictionary_recursively(h5file,path + key + '/',value)
            else:
                # Prepare the data for storage
                data = value
                ...
                # Handle empty arrays or lists
                if isinstance(value, (list, np.ndarray)) and len(value) == 0:
                    # Decide how to handle empty arrays; here we store an empty dataset
                    h5file.create_dataset(path + key, data=np.array([]))
                    continue

                # Handle scalar values and unsupported types

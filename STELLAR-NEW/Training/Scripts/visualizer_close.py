...
def write2text(chaser, data_dir, file_name, step, opt_id):
    ...
    """
    # Use the provided directory as is if it's an absolute path;
    ...
    # Get the folder in which the file will reside.
    ...
    # Retrieve the trace data from the chaser.
    ...
    if opt_id == 0:
        ...
        pos_arr = actuation_arr[step-2:, :]
    else:
        raise Exception(f'INVALID INPUT {opt_id} for opt_id in write2text. Valid options are 0, 1, or 2.')
    
    # Format each line as "x,y,z"
    lines2write = ['\n' + ','.join(str(x) for x in pos) for pos in pos_arr]

...
def evaluate(model, num_episodes=1000):
    ...
    for i in trange(num_episodes,  ncols= 1000, mininterval=0.3, desc="Evaluating"):
    #for i in tgrange(num_episodes,  ncols= 1000, mininterval=0.3, gui=True, desc="Evaluating"):
        ...
        obs = env.reset() # znv.reset seems where the chaser.txt fues are created: need to check.
        print('RESET')
        ep_steps = 0
        fuel_consumed = -1
        while not done:
            ...
            #print('PRIOR STEP')
            obs, reward, done, info = env.step(action) # at every step of the chaser.txt file?
            #print('TEST_STEP') # Problem in the env.step. NEED TO FIX!!!!!
       ...
       # print(all_terminalstates)
       # print(unnormalized_state)
...
env = make_vec_env(ARPOD_GYM, wrapper_class=TimeLimitWrapper, n_envs=1,env_kwargs={'log_dir': 'Benchmark'})
...
modelname = 'finetuned_model'
model_dir = f'Training/Models/{modelname}'
...
if usr_policy == 'y':
    policy_args = {'activation_fn': GELU, 'net_arch': {'pi': [130, 44, 30], 'vf': [145, 25, 5]}, 
                   'full_std': False, 'squash_output': False, 'log_std_init': 1.41421356237, 
                   'ortho_init': True, 'use_expln' : True}
    #policy_dir = 'models/vbar-agentv3-3.zip'
    #policy_dir = 'Training/Models/finetuned_model'
    #policy_dir = 'vbar-agentv3'
    #policy_dir = 'Training/envstats/vbar-agent_300000_steps'
    #policy_dir = 'Training/envstats/vbar-agent_2250000_steps.zip'
    policy_dir = 'Training/Models/vbar-agent.zip'




    fresh_model = PPO.load(policy_dir, env=normalized_vec_env, print_system_info=True, verbose=2, 
                           policy_kwargs=policy_args, _init_setup_model=False, force_reset=True, device='cuda')

...
class ARPOD_GYM(gym.Env):
    ...
    def __init__(self, log_dir = 'Training'):
        ...
        runs_base = os.path.join(log_dir, "runs")
        if not os.path.exists(runs_base):
            os.makedirs(runs_base)

        vels_base = os.path.join(log_dir, "velocities")
        if not os.path.exists(vels_base):
            os.makedirs(vels_base)

        acts_base = os.path.join(log_dir, "actuations")
        if not os.path.exists(acts_base):
            os.makedirs(acts_base)



        common_model_id = len(os.listdir(runs_base))  # This assumes you want consistency with the runs folder.

        # Use the same common_model_id for all three.
        self.runs_dir = os.path.join(runs_base, f"vbar{common_model_id}")
        os.makedirs(self.runs_dir, exist_ok=True)

        self.vels_dir = os.path.join(vels_base, f"vbar{common_model_id}")
        os.makedirs(self.vels_dir, exist_ok=True)

        self.act_dir = os.path.join(acts_base, f"vbar{common_model_id}")
        os.makedirs(self.act_dir, exist_ok=True)

    """     model_id = len(os.listdir(runs_base))
        self.runs_dir = os.path.join(runs_base, f"vbar{model_id}")
        os.makedirs(self.runs_dir, exist_ok=True)

        model_idv = len(os.listdir(vels_base))
        self.vels_dir = os.path.join(vels_base, f"vbar{model_idv}")
        os.makedirs(self.vels_dir, exist_ok=True)

        model_idact = len(os.listdir(acts_base))
        self.act_dir = os.path.join(acts_base, f"vbar{model_idact}")
        os.makedirs(self.act_dir, exist_ok=True) """

    """ 
        model_id = len(os.listdir(os.path.join(log_dir, "runs")))
        self.runs_dir = os.path.join(log_dir, "runs", f"vbar{model_id}")
        os.makedirs(self.runs_dir, exist_ok=True)
 
        # For the velocities folder:
        model_idv = len(os.listdir(os.path.join(log_dir, "velocities")))
        self.vels_dir = os.path.join(log_dir, "velocities", f"vbar{model_idv}")
        os.makedirs(self.vels_dir, exist_ok=True)

        # For the actuations folder:
        model_idact = len(os.listdir(os.path.join(log_dir, "actuations")))
        self.act_dir = os.path.join(log_dir, "actuations", f"vbar{model_idact}")
        os.makedirs(self.act_dir, exist_ok=True)
    """

    """         model_id = len(os.listdir('Benchmark/runs/'))
        ...
        model_idv = len(os.listdir('Benchmark/velocities/'))
        self.vels_dir = f'vbar{model_idv}'
        os.makedirs(self.vels_dir, exist_ok=True)
        print(f'creating vels log in directory {self.vels_dir}')

        model_idact = len(os.listdir('Benchmark/actuations/'))
        self.act_dir = f'vbar{model_idact}'
        os.makedirs(self.act_dir, exist_ok=True) 
        print(f'creating actuation logs in directory {self.act_dir}')
    """
    ...
    def step(self, action):
        ...
        r, flag_docked = self.r_form.win_conditions()
        reward += r
        
        if flag_docked:

            if len(self.info['collision_state']) <= 1 and len(self.info['docked_state']) <= 1:
                print(
                    f'chaser docked at {self.chaser.state} at step {self.chaser.current_step}'
                )
                self.info['docked_state'] = np.copy(obs)
                self.info['docked_step'] = self.chaser.current_step + (self.info['docked_step'] + 1 )
            
            return obs, reward, terminated, truncated, self.info
	    #checking collision / phase 3 terminal constraints
        p, collision = self.r_form.truncate_conditions()
        reward += p
        #print('REWARD!!!!!!!!!!')
        if collision:
            if len(self.info['collision_state']) <= 1 and len(self.info['docked_state']) <= 1:
                print(
                    f'chaser collided at {self.chaser.state} at step {self.chaser.current_step}'
                )
                self.info['collision_state'] = np.copy(obs)
                self.info['collision_step'] = self.chaser.current_step + (self.info['collision_step'] + 1 )
            return obs, reward, terminated, truncated, self.info
    ...
    def write_data(self, file_name):
        ...
        write2text(self.chaser, self.runs_dir, file_name, self.chaser.current_step, 0)
        write2text(self.chaser, self.vels_dir, file_name, self.chaser.current_step, 1)
        write2text(self.chaser, self.act_dir, file_name, self.chaser.current_step, 2)
